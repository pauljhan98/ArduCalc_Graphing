...
bool exit_flag = false;         // Flag active when user presses esc key; for the graph function
...
void makePlot(Adafruit_ILI9341 *tft_point, long int start_address, long int end_address, float *output_point, SpiRAM spiram, int arraysize) {

  drawAxis(tft_point);   // Make the screen blank, and plot the numbered axis

  // Next step: we look into the offboard SRAM to retrieve the x and y values
  ...
  for (long i = start_address; i <= end_address; i += 8) {
    ...
    // Fix the displayed graph discontinuity 
    if ((abs(prior_y - y) > 1) && (pixel_counter > 1)) {
      // first, make sure y is in bounds with the screen, otherwise smoothing stuff goes on plotting for a very long time!

    //  if (abs(prior_y/WEIGHT) > (WEIGHT*160)) {
       for (int z = prior_y; z < y; z++) {
        tft_point->drawPixel(z, pixel_counter - 1, ILI9341_RED);
       }
       for (int z = y; z < prior_y; z++) {
        tft_point->drawPixel(z, pixel_counter - 1, ILI9341_RED);
       }
    //  }
    }
    ...
    prior_y = y;
  Serial.println("plotting rn");
    ///
  }
  Serial.println("End graphing");
  tft_point->setRotation(3);
  tft_point->endWrite();

  while (exit_flag == false) {                     // If the user hits the escape key, it leaves the graph
    while (keyboard.available()) {                 // Can be modified and expanded later
