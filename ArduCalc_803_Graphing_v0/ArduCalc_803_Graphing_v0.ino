  ...
  if (gui.update_screen(gui_output_ptr, gui_buffer_size)) {        //gui_output_ptr
    if (input_parser.parse_input(gui_output_ptr)) {
      if (arithmetic_logic.calculate(start_address, end_address) == true) {
        record_answer();
        makePlot(tft_point, output_data[output_data_count].table_start_address, output_data[output_data_count].table_end_address, output_buff , SpiRam, 8); //display graph
      } 
      else {
        record_answer();
      }
