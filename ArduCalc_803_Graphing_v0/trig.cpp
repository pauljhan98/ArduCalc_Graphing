...
double tan_values[TAN_VALUES] {         // Pre-calculated arc-tangent values for use in the CORDIC function
               //  1.57079632679,
                .7853981634,
                .463647609,
                .2449786631,
...
void cordicTrig(float angle, float *cosine, float *sine) {
 ...
 for (int i = 0; i <= TAN_VALUES; i++) {                       // Preform rotations to try and get the intial vector
  if (accumulated_angle >= angle) {                            // to the same angle as the input angle
    xval_prior = xval;
    xval = xval + (yval*pow(2, -i));                           // "Bitshift" to get x and y to correct values
