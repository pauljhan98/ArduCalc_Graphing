...
// Input parsing private variables
...
// Input Parsing external variables
...
bool Input_Parser::parse_input(char*& gui_input_ptr) {      //RPN input style, format e.g. -12,34.56,+,-78.09,- //-,+,*,^,/ operators supported
  newInput = false;
  while (*gui_input_ptr != '\0') {                          // still recieving keypresses for current input    
    RawInput = *gui_input_ptr - '0';                        // convert from ASCII value to numerical value
    switch (RawInput) {
      ...
      case '+'-'0': case '^'-'0': case '*'-'0':  case '/'-'0': case 's'-'0': case 'c'-'0': case 't'-'0': //+,*,/,^ operators
        stack.push(RawInput);
